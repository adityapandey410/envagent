...
def plan_node(state: AgentState) -> AgentState:
    ...
    shell_note = (
        ...
        "\"check_command\" on this OS: a bare boolean expression's printed "
        "value (`Test-Path $x`, `$a -contains $b`, a comparison) does NOT "
        "set the process exit code in Windows PowerShell — the process "
        "exits 0 regardless of whether the expression is True or False, so "
        "such a check_command would ALWAYS be treated as \"already "
        "satisfied\" even when it prints False (confirmed live: this "
        "silently made a real run skip a step it still needed). Always "
        "wrap a Windows check_command's condition explicitly: "
