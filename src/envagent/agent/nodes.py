...
def plan_node(state: AgentState) -> AgentState:
    ...
    shell_note = (
        ...
        "instead), no heredocs, no `$(...)` command substitution. When combining "
        "a cmdlet call with `-and`/`-or`, always wrap the cmdlet call in "
        "parentheses: `(Test-Path $x) -and $y`, NOT `Test-Path $x -and $y` — "
        "PowerShell parses the latter as passing `-and` as a named parameter to "
        "Test-Path itself, which errors. A *persistent* PATH/environment change "
        "(`setx`, or `[Environment]::SetEnvironmentVariable(..., \"User\")`/"
        "`\"Machine\"`) IS visible to later steps in this plan — this agent "
        "re-reads the registry before every command specifically so that "
        "works; prefer it over a session-only `$env:PATH = ...` change "
        "whenever a later step needs the result. CRITICAL for every "
        "\"check_command\" on this OS: in Windows PowerShell, a cmdlet's or "
        "expression's own result — a bare boolean (`Test-Path $x`, "
        "`$a -contains $b`, a comparison) or a cmdlet's match/no-match "
        "state (e.g. piping through `Select-String`) — does NOT set the "
        "process exit code. The process exits 0 regardless of that result "
        "unless something explicitly sets it, so such a check_command "
        "would ALWAYS be treated as \"already satisfied\" even when the "
        "real condition is false (confirmed live: this silently made a "
        "real run skip a step it still needed). Never rely on a bare "
        "expression or an un-checked pipeline for a Windows "
        "check_command — always end it with an explicit "
